...
async function fetchApp() {
  ...
  if (!club) {
    const cRes = await sb.from("clubs").select("*").eq("status", "active").order("created_at").limit(1);
    club = cRes.data && cRes.data[0];
  }
...
const DEMO_CLUB = { id: "demo-club", name: "Cypress Hollow Country Club", slug: "demo", status: "active", theme: { tagline: "Est. 1931 · A Fictional Club · ClubMajors Demo", pros: [{ first: "Chip", last: "Anderson" }, { first: "Danny", last: "Noonan" }] } };
...
function ClubMajorsPrototype() {
  ...
  const [noLogo, setNoLogo] = useState(false); /* club chooses to show its name only, no crest */
